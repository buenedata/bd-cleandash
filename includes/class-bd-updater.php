...
if (!class_exists('BD_CleanDash_Updater')) {
class BD_CleanDash_Updater {
    private $plugin_file;
    ...
    public function __construct($plugin_file, $github_username, $github_repo) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("BD_CleanDash_Updater initialized for {$this->plugin_basename} (slug: {$this->plugin_slug}, version: {$this->version})");
        }
    }
    ...
    public function plugin_info($result, $action, $args) {
        ...
        if (!is_wp_error($request) && wp_remote_retrieve_response_code($request) === 200) {
            ...
            $result = (object) [
                ...
                'short_description' => 'BD CleanDash Plugin fra Buene Data',
                'sections' => [
                    'description' => wpautop(wp_kses_post($data['body'] ?? 'Clean up your WordPress dashboard by hiding unwanted notices and widgets.')),
